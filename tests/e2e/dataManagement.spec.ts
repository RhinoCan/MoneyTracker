...
test('Restore All Settings to Defaults resets locale to American English', async ({ page }) => {
  ...
  await openSettings(page);
  await expect(page.locator('[data-testid="locale-select"]')).toContainText('en-US', { timeout: 5000 });
...
// -------------------------------------------------------------------------
test('Delete All Transactions and Settings clears transactions and resets settings', async ({ page }) => {
