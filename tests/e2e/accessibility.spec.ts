...
async function runAxe(page: Page) {
  ...
  if (minor.length > 0) {
    console.warn('Minor/moderate axe violations:', JSON.stringify(minor.map(v => ({
      id: v.id,
      impact: v.impact,
  ...
  expect(serious, `Axe critical/serious violations: ${JSON.stringify(serious.map(v => ({
    id: v.id,
    impact: v.impact,
    description: v.description,
...
test.describe('Accessibility', () => {
  ...
  test('settings dialog — InfoIcon visible (timeout mode)', async ({ page }) => {
    ...
    await login(page);
    await openSettings(page);
    // Ensure persistent messages is OFF so the slider and InfoIcon are visible
  ...
  test('delete transaction dialog', async ({ page }) => {
    ...
    try {
      ...
      await page.getByRole('dialog').locator('button').filter({ hasText: /cancel/i }).click();
